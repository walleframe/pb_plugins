...
var UserOP = func() UserOperation {
 ...
 - [表名]SelectWhere / [表名]CountWhere / [表名]DeleteWhere 自定义拼接sql语句辅助. 使用?占位符.

辅助常量和变量:
 - [表名]SQL_Create 常量 创建sql表语句
 - [表名]SQL_TableColumns map变量. 修改表添加某个字段的语句.
 - [表名]SQL_TableIndex map变量. 创建索引的sql语句.
 - [表名]SQL_Find / [表名]SQL_Count / [表名]SQL_Delete 辅助语句, 自定义查询时候可以使用.
...
var (
	...
	query1 = db_pff.UserSelectWhere(128).Id().Equal().And().Name().Equal().String()
	query2 = db_pff.UserSQL_Find + " where `id` = ? and `name` = ?"
)
#+end_src
